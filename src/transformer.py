...
class Transformer(nn.Module):
    ...
    def forward(self, src_input: torch.Tensor, tgt_input: torch.Tensor, attn_mask: torch.Tensor = None) -> torch.Tensor:
        ...
        enc_output = self.encoder(src_input, mask=attn_mask)
        ...
        for _ in range(max_length):
            ...
            for seq, score in beam:
                ...
                for next_token in range(log_probs.size(1)):
                    ...
                    new_score = new_score = score + log_probs[0, next_token].item()
    ...
    def __top_k_sampling_decode(self, src_input: torch.Tensor, max_length: int, k: int = 10, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            topk_log_probs, topk_indices = torch.topk(logits, k, dim=-1)
    ...
    def __contrastive_decode(self, src_input: torch.Tensor, max_length: int, k: int = 5, alpha: float = 0.6, **kwargs) -> torch.Tensor:
        ...
        for _ in range(max_length):
            ...
            expanded_tgt_input = tgt_input.expand(k, -1)  # Shape: (k, seq_len)
            ...
            dec_outputs_candidate = self.decoder(y_candidates, enc_output.expand(k, enc_output.size(1), enc_output.size(2)))
            ...
            scores = alpha * torch.log(P_LM_v.clamp_min(1e-12)) - (1 - alpha) * max_sim  # Shape: (k,)

            # Select the candidate with the highest score
            best_idx = torch.argmax(scores)
            best_token = next_tokens[best_idx:best_idx+1]  # Shape: (1, 1)
